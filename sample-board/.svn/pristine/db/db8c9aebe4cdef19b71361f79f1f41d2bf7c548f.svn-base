...
public class QnaDaoProvider {
	//2. 싱클톤을 적용할 클래스를 생성해서 인스턴스로 저장한다.
	private static QnaDaoProvider instance
	= new QnaDaoProvider();
	//3. 생성된 인스턴를 받아가기 위한 getter를 만든다.
	public static QnaDaoProvider getInstance(){
//		System.out.println
//		("MessageDaoProvider.getInstance()");
		return instance;
	}
	//1. 생성자를 private 으로 선언해 다른 클래스의 생성금지
	private QnaDaoProvider(){}
	
	// 사용하고자 하는 모든 dbms의 dao 프로그램을 생성해서 저장해 놓는다.
	private MySQLQnaDao mysqlDao 
	= new MySQLQnaDao();
	private OracleQnaDao oracleDao 
	= new OracleQnaDao();
	private MSSQLQnaDao mssqlDao 
	= new MSSQLQnaDao();
